...
describe("parseXUrl", () => {
  it("accepts x.com status URLs", () => {
    ...
    assert.equal(r.loadUrl, "https://x.com/i/status/1949680387330027593");
  });
  ...
  it("accepts twitter.com status URLs and tracking params", () => {
    ...
    assert.equal(r.ok, true);
    assert.equal(r.loadUrl, "https://x.com/i/status/1234567890123456789");
  });
  ...
  it("accepts /video/1 suffixes and mobile hosts", () => {
    ...
    assert.equal(r.ok, true);
    assert.equal(r.loadUrl, "https://x.com/i/status/1112223334445556667");
  });
  ...
  it("accepts protocol-less hosts and bare ids", () => {
    ...
    assert.equal(parseXUrl("1814440131505598541").loadUrl, "https://x.com/i/status/1814440131505598541");
  });
